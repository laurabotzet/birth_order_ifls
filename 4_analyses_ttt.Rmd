...
plot_birthorder = function(model, ylabel = NULL, title = "", bo_var = "birth_order", separate = TRUE) {
  ...
  plotx = ggplot(cemm, aes(`Birth order`, y = fit, ymax = upper, ymin = lower,
    ...
    geom_pointrange(stat = "identity", position = position_dodge(width = 0.5), size = 2) +
    ...
    scale_y_continuous(name = ylabel, limits = c(-0.8, 0.9),
...
model = lmer(outcome ~ poly(age, 3, raw = TRUE) + male +
               ...
               (1 | mother_pidlink),
                   data = birthorder)
ylabel = "Intelligence (z-standardized)"

plot_birthorder(model, ylabel=ylabel) 
...
model = lmer(outcome ~ poly(age, 3, raw = TRUE) + male +
               ...
               (1 | mother_pidlink),
                   data = birthorder)
ylabel = "Years of education (z-standardized)"
...
model = lmer(outcome ~ poly(age, 3, raw = TRUE) + male +
               ...
               (1 | mother_pidlink),
                   data = birthorder)
ylabel = "Agreeableness (z-standardized)"
...
model = lmer(outcome ~ poly(age, 3, raw = TRUE) + male +
               ...
               (1 | mother_pidlink),
                   data = birthorder)
ylabel = "Extraversion (z-standardized)"
...
model = lmer(outcome ~ poly(age, 3, raw = TRUE) + male +
               ...
               (1 | mother_pidlink),
                   data = birthorder)
ylabel = "Conscientiousness (z-standardized)"
...
model = lmer(outcome ~ poly(age, 3, raw = TRUE) + male +
               ...
               (1 | mother_pidlink),
                   data = birthorder)
ylabel = "Openness (z-standardized)"
...
model = lmer(outcome ~ poly(age, 3, raw = TRUE) + male +
               ...
               (1 | mother_pidlink),
                   data = birthorder)
ylabel = "Neuroticism (z-standardized)"
...
model = lmer(outcome ~ poly(age, 3, raw = TRUE) + male +
               ...
               (1 | mother_pidlink),
                   data = birthorder)
ylabel = "Risk Aversion A (z-standardized)"
...
model = lmer(outcome ~ poly(age, 3, raw = TRUE) + male +
               ...
               (1 | mother_pidlink),
                   data = birthorder)
ylabel = "Risk Aversion B (z-standardized)"
