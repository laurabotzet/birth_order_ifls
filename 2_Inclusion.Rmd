...
```{r}
missingness_patterns(alldata_birthorder %>% select(g_factor, birthorder_naive, old_enough, mother_pidlink))
```
...
alldata_birthorder = alldata_birthorder %>%
  ...
  select(-ends_with("_ans"), -starts_with("co0"), -co10count, -starts_with("cob"), -matches("[a-z][0-9]r?"),
         -matches("[a-z][0-9]r_reversed"), -matches("si[0-9]"), -sex.y, -birthdate_duped_in_earlier_wave,
