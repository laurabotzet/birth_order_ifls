...
```{r}
birthorder %>% filter(!is.na(birthorder_genes), group_outcome == 1) %>% group_by(mother_pidlink) %>% summarise(with_data = n(), all = mean(sibling_count_genes)) -> counts
birthorder %>% filter(!is.na(birthorder_genes), group_outcome == 0) %>% group_by(mother_pidlink) %>% summarise(with_data = n(), all = mean(sibling_count_genes)) -> counts1
```
...
## Ttest
t.test(birthorder$age ~ birthorder$group_birthorder, var.equal = T)
...
t.test(birthorder$g_factor_2015_old ~ birthorder$group_birthorder, var.equal = T)
t.test(birthorder$big5_ext ~ birthorder$group_birthorder, var.equal = T)
t.test(birthorder$big5_neu ~ birthorder$group_birthorder, var.equal = T)
t.test(birthorder$big5_con ~ birthorder$group_birthorder, var.equal = T)
t.test(birthorder$big5_agree ~ birthorder$group_birthorder, var.equal = T)
t.test(birthorder$big5_open ~ birthorder$group_birthorder, var.equal = T)
t.test(birthorder$riskA ~ birthorder$group_birthorder, var.equal = T)
t.test(birthorder$riskB ~ birthorder$group_birthorder, var.equal = T)
...
## Correlations
```{r}
round(cor(birthorder %>% ungroup() %>% select(age, male, g_factor_2015_old, big5_ext, big5_neu, big5_con, big5_agree, big5_open, riskA, riskB, years_of_education), use = "pairwise.complete.obs"), 2) 
```
