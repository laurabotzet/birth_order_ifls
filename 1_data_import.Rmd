...
```{r compare mother information with our information}
...
parentscheck = parentscheck %>%
  filter(!is.na(mother_pidlink.y)) %>%
  filter(!is.na(mother_pidlink.x)) %>%
  filter(!is.na(mother_birthdate))

parentscheck = parentscheck %>%
    mutate(unique = 
               paste0(mother_birthdate, "-", pidlink))

parentscheck = subset(parentscheck,!duplicated(parentscheck$unique))

parentscheck = parentscheck %>% 
   group_by(mother_pidlink.y) %>%
   mutate(siblingcount2 = n())

ggplot(parentscheck, aes(x=siblingcount2, y=total_number_childs_m)) + geom_jitter()

individuals_parents_checked = subset(parentscheck, parentscheck$age>14)

individuals_parents_checked = individuals_parents_checked %>%
  group_by(mother_father_dyad) %>%
  mutate(birthorder = min_rank(birth)) %>%
  group_by(mother_father_dyad, birth) %>%
  mutate(multiple_birth = n()) %>%
