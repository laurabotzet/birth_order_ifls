...
alldata_pregnancy %>% filter(birthorder_naive != birthorder_uterus_alive) %>% select(birthorder_naive,birthorder_naive_ind, birthorder_uterus_alive, sibling_count_uterus_alive, mother_pidlink, birthdate, mother_birthdate) %>% View()
...
=======
iq <- rename(iq, age = IQage) 
>>>>>>> 2783301dafb78ad19b264e61581a0c5b386da73e
...
iq$count_backwards[rowSums( is.na(answered_counting_items)) == 5] = NA_real_
>>>>>>> 2783301dafb78ad19b264e61581a0c5b386da73e
...
iq2007_ek1_repeater = iq2007_ek1_repeater %>% select(pidlink, raven_2007_young_repeater,
...
iq %>% select(raven, math, raven_young_2014, math_young_2014, count_backwards, words_immediate, words_delayed, adaptive_numbering, raven2007, math2007) %>% tidyr::gather() %>% 
...
=======
"g_factor =~ raven + math + count_backwards +  words_delayed + adaptive_numbering + raven_young_2014 + math_young_2014 + raven2007 + math2007" %>%
>>>>>>> 2783301dafb78ad19b264e61581a0c5b386da73e
...
iq = iq %>% rename(IQ_age = age.x) %>% select(-age.y)
>>>>>>> 2783301dafb78ad19b264e61581a0c5b386da73e
```
...
formr::missingness_patterns(alldata_birthorder %>% select(raven, math, raven_young_2014, math_young_2014, count_backwards, words_immediate, words_delayed, adaptive_numbering, raven2007, math2007, g_factor))
...
formr::missingness_patterns(alldata_birthorder %>% 
   select(mother_pidlink, birthorder_naive, count_backwards, raven, math, words_delayed, adaptive_numbering))

formr::missingness_patterns(alldata_birthorder %>% 
   select(mother_pidlink, birthorder_naive, raven, big5_ext, riskA, age,old_enough))
formr::missingness_patterns(alldata_birthorder %>% 
   select(mother_pidlink, birthorder_uterus_alive, g_factor, big5_ext, riskA, age,old_enough))
formr::missingness_patterns(alldata_birthorder %>% 
