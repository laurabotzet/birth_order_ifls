...
## test
```{r}
x = birthorder %>% filter(!is.na(g_factor), !is.na(birthorder_uterus_alive), !is.na(age), !is.na(male), !is.na(sibling_count_uterus_alive), !is.na(mother_pidlink), sibling_count_uterus_alive != 1)

compare = readRDS("data/birthorder.rds")

compare = compare %>% select(pidlink, g_factor_old = g_factor)

x = left_join(x, compare, by = "pidlink")

y = x %>% filter(is.na(g_factor_old))
skimr::skim(y)
qplot(x$g_factor, x$g_factor_old)
```
