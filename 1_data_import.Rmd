...
prop.table(table(pregnancy$ch06a))

## Rename Variables
pregnancy = pregnancy %>% rename(lifebirths = ch06, multiple_birth = ch06a, gender = ch08, birth_day = ch09day, birth_month = ch09mth, birth_year = ch09yr, mother_pidlink = pidlink, alive = ch25) # pregnancy$lifebirths values: 1 = still pregnant, 2 = livebirth, 3 = still birth, 4 = misscarriage
...
### Mark all families/individuals with multiple births for pregnancy order:
## All women that ever had multiple births
twinmothers_preg = pregnancy %>%
  # born in the same month
  mutate(multiple_birth_preg = ifelse(duplicated(mother_birthdate) & !is.na(birth_month), 1, 0)) %>% 
  filter(multiple_birth_preg == 1) %>% 
  select(mother_pidlink, mother_birthdate, multiple_birth_preg)
table(duplicated(twinmothers_preg$mother_pidlink))
nrow(twinmothers_preg)
## Mark individuals with multiple births in family
pregnancy = pregnancy %>% left_join(
  twinmothers_preg %>% select(mother_birthdate, multiple_birth_preg) %>% unique(), 
  by = "mother_birthdate") %>% 
  left_join(
  twinmothers_preg %>% select(mother_pidlink, multiple_birth_preg) %>% rename(any_multiple_birth_preg = multiple_birth_preg) %>% unique(), 
  by = "mother_pidlink") %>% 
  mutate(multiple_birth_preg = if_else(multiple_birth_preg==1, 1, 0, 0),
         any_multiple_birth_preg = if_else(any_multiple_birth_preg==1, 1, 0, 0))

prop.table(table(pregnancy$multiple_birth_preg))
prop.table(table(pregnancy$any_multiple_birth_preg))
...
twinmothers = pregnancy %>%
  filter(lifebirths == 2) %>%
  # born in the same month
  mutate(multiple_birth = ifelse(duplicated(mother_birthdate) & !is.na(birth_month), 1, 0)) %>% 
  filter(multiple_birth == 1) %>% 
  select(mother_pidlink, mother_birthdate, multiple_birth)
table(duplicated(twinmothers$mother_pidlink))
nrow(twinmothers)
## Mark individuals with multiple births in family
pregnancy = pregnancy %>% left_join(
  twinmothers %>% select(mother_birthdate, multiple_birth) %>% unique(), 
  by = "mother_birthdate") %>% 
  left_join(
  twinmothers %>% select(mother_pidlink, multiple_birth) %>% rename(any_multiple_birth = multiple_birth) %>% unique(), 
  by = "mother_pidlink") %>% 
  mutate(multiple_birth = if_else(multiple_birth == 1, 1, 0, 0),
         any_multiple_birth = if_else(any_multiple_birth == 1, 1, 0, 0))
