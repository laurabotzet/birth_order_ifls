...
library(cowplot)
plots_nonadjusted = plot_grid(plot_gfactor, plot_yoe, plot_big5_ext, plot_big5_neu, plot_big5_con,
                     plot_big5_agree, plot_big5_open, plot_riskA, plot_riskB,
                     ncol = 3,
                     labels = c("(a)", "(b)", "(c)", "(d)", "(e)", "(f)", "(g)", "(h)", "(i)"))

pdf(file = "Submission EJP/Botzet_Figure2.pdf",   # The directory you want to save the file in
    width = 15, # The width of the plot in inches
    height = 17)
plots_nonadjusted
dev.off()

svg(file = "Submission EJP/Botzet_Figure2.svg",   # The directory you want to save the file in
    width = 15, # The width of the plot in inches
    height = 17)
plots_nonadjusted
dev.off() 

jpeg(file = "Submission EJP/Botzet_Figure2.jpg",   # The directory you want to save the file in
    width = 1500*0.8, # The width of the plot in inches
    height = 1700*0.8,
    quality = 200)
plots_nonadjusted
dev.off() 
...
library(gridExtra)
plots_adjusted = grid.arrange(plot_gfactor, plot_yoe, plot_big5_ext, plot_big5_neu,
                              plot_big5_con,
                     plot_big5_agree, plot_big5_open, plot_riskA, plot_riskB,
                     ncol = 3)
```
...
```{r g-factor 2015 old}
`Risk A` = lmer(riskA ~ birthorder_genes + poly(age, 3, raw = TRUE) + male +
                                  sibling_count_genes + (1 | mother_pidlink),
                   data = birthorder)
...
```{r g-factor 2015 old}
`Risk B` = lmer(riskB ~ birthorder_genes + poly(age, 3, raw = TRUE) + male +
                                  sibling_count_genes + (1 | mother_pidlink),
                   data = birthorder)
...
## Multiplot - linear
```{r}
x = multiplot(Intelligence, `Educational attainment`, Agreeableness, Conscientiousness, Extraversion, Neuroticism, Openness, `Risk A`, `Risk B`, intercept = FALSE, coefficients = c("birthorder_genes", "sibling_count_genes"), plot = F)
        
x = data.frame(x)

x$Model = ifelse(x$Model == "`Educational attainment`", "Educational attainment", x$Model)
x$Model = ifelse(x$Model == "`Risk A`", "Risk aversion A", x$Model)
x$Model = ifelse(x$Model == "`Risk B`", "Risk aversion B", x$Model)

x$Model = as.factor(x$Model)
levels(x$Model)

x$Coefficient = ifelse(x$Coefficient == "birthorder_genes", "Birth order", "Sibship size")
x$Coefficients = x$Coefficient

plot = ggplot(x, aes(Model, Value)) +
  geom_pointrange(data=x, mapping=aes(x=Model, y=Value, ymin=HighOuter, ymax=LowOuter, color = Coefficients),
                  position = position_dodge(width=0.3), size = 1) +
  scale_colour_grey() +
  geom_hline(yintercept = 0, size = 1.5)  +
  geom_hline(yintercept = -.1, linetype = "dotted", size = 1.5) + 
  scale_x_discrete(limits=c("Risk aversion B", "Risk aversion A", "Openness", "Neuroticism", "Extraversion",
  ...
  theme(text = element_text(size=15), axis.text.x = element_text(size = 10),
          ...
          legend.background = element_rect(fill="gray90", size=.5, linetype="dotted"),
          plot.title = element_text(hjust = 0), legend.key.size = unit(2, 'line')) +
  labs(y = "Linear effect size", x = "Outcome")


pdf(file = "Submission EJP/Botzet_Figure1.pdf",   # The directory you want to save the file in
    width = 15, # The width of the plot in inches
    height = 17)
plot
dev.off()

svg(file = "Submission EJP/Botzet_Figure1.svg",   # The directory you want to save the file in
    width = 15, # The width of the plot in inches
    height = 17)
plots_nonadjusted
dev.off() 

jpeg(file = "Submission EJP/Botzet_Figure1.jpg",   # The directory you want to save the file in
    width = 1500*0.8, # The width of the plot in inches
    height = 1700*0.8,
    quality = 200)
plots_nonadjusted
...
## Multiplot - nonlinear
```{r}
x = multiplot(Intelligence, `Educational Attainment`, Agreeableness, Conscientiousness, Extraversion, Neuroticism, Openness, RiskA, RiskB, intercept = FALSE, coefficients = c("birthorder_genes_factor2", "birthorder_genes_factor3", "birthorder_genes_factor4", "birthorder_genes_factor5", "birthorder_genes_factor>5"), plot = F)
                       ...
                       ifelse(x$Coefficient == "birthorder_genes_factor3", "3",
                              ifelse(x$Coefficient == "birthorder_genes_factor4", "4",
                                     ifelse(x$Coefficient == "birthorder_genes_factor5", "5",
                                            ifelse(x$Coefficient == "birthorder_genes_factor>5", ">5", NA)))))
