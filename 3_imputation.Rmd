...
alldatasm <-  alldata_birthorder %>% arrange(miss_frac_person) %>% slice(1:200)
...
# mother_pidlink predicts 29 other variables
table(predictor_matrix[, "mother_pidlink"] == 0)

# to make the imputation possible, we exclude all "unimportant" variables from the imputation by mother_pidlink

ini = mice(alldatasm, maxit = 0, predictorMatrix = predictor_matrix)
pred <- ini$pred
