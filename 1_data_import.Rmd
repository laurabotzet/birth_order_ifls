...
fa(nomiss %>% select(raven_2015_old, math_2015_old, count_backwards, words_delayed, adaptive_numbering) %>% data.frame(), nfactors = 2)
om_results = omega(nomiss %>% select(raven_2015_old, math_2015_old, count_backwards, words_delayed, adaptive_numbering) %>% data.frame(), nfactors = 2, sl = F)
om_results
omega.diagram(om_results)

"g_factor_early =~ raven_2015_old + math_2015_old + count_backwards +  words_delayed+  adaptive_numbering" %>%
  cfa(data = nomiss, std.lv = T, std.ov = T) -> cfa_g
summary(cfa_g)

nomiss$g_factor_early = predict(cfa_g)[,1]

qplot(nomiss$g_factor_early)

nomiss = nomiss %>% select(pidlink, g_factor_early)
...
alldata_birthorder <- alldata_birthorder %>% 
  ...
  mutate_at(vars(birthdate), funs(as.Date)) %>% 
    select(-motherID, -fatherID, -mother_birthorder, -mother_birthdate, -month, -end, -start, -gender, -sex, -sex_2015_old, -sex_2015_young,
           ...
           -money_spent_smoking, -starts_with("hhid14_"))
```
