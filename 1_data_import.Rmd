...
### Individuals
individuals = bk_ar1 %>% select(hhid14_9, pidlink, father_pidlink, mother_pidlink, ar02b, ar10, ar11, ar07, ar08day, ar08mth, ar08yr, ar09, ar18h)
#Rename variables to make it easier
individuals <- plyr::rename(individuals, c("ar02b"="relation_to_HH_head", "ar10"="fatherID", "ar11" = "motherID", "ar07" = "sex", "ar09" = "age", "ar18h" = "alive")) 
# Remove duplicats:
...
#Rename variables to make it easier
motherinformations <- plyr::rename(motherinformations, c("ba63cx"="biologicalchild_m", "ba64"="sex_m", "ba64bmt" = "birth_month_m", "ba64byr" = "birth_year_m", "ba64c" = "living_in_HH_m", "ba65" = "alive_m", "ba65mt"= "death_month_m", "ba65ayr" = "death_year_m", "ba66" = "death_age_m", "ba63n" = "total_number_childs_m", "pidlink"="mother_pidlink")) 

# Mothercheck
individuals$check = ifelse(individuals$mother_pidlink %in% motherinformations$mother_pidlink, 1, 0)
...
motherinformations = motherinformations %>%
    mutate(mother_birthdate = 
           ifelse(is.na(birth_year_m), NA,
                  ifelse(is.na(mother_pidlink), NA,
                         ifelse(is.na(birth_month_m), NA, 
                         paste0(mother_pidlink, "-", birth_year_m, "-", birth_month_m)))))
...
individuals = individuals %>%
    mutate(mother_birthdate = 
           ifelse(is.na(ar08yr), NA,
                  ifelse(is.na(ar08mth), NA,
                     ifelse(is.na(mother_pidlink), NA,
                           paste0(mother_pidlink, "-", ar08yr, "-", ar08mth)))))

individuals = individuals %>%
  filter(!is.na(mother_birthdate)) %>%
  subset(individuals$check == 1)


parentscheck = full_join(individuals, motherinformations, by = "mother_birthdate")

parentscheck$mother_pidlink = ifelse(!is.na(parentscheck$mother_pidlink.y), parentscheck$mother_pidlink.y, ifelse(!is.na(parentscheck$mother_pidlink.x), parentscheck$mother_pidlink.x, NA))

parentscheck = parentscheck %>%
  filter(!is.na(mother_birthdate)) %>%
  filter(!is.na(mother_pidlink))

parentscheck = subset(parentscheck,!duplicated(parentscheck$mother_birthdate))


### Biological Birthorder: Order in Uterus
# only include biological childs
parentscheck_biological = parentscheck
parentscheck_biological$biologicalchild_m = ifelse(is.na(parentscheck_biological$biologicalchild_m), 1, parentscheck_biological$biologicalchild_m)
parentscheck_biological = parentscheck_biological %>%
  filter(biologicalchild_m==1)

parentscheck_biological = parentscheck_biological %>% 
   group_by(mother_pidlink) %>%
   mutate(siblingcount2 = n())

parentscheck_biological = parentscheck_biological %>%
    mutate(birth = 
           ifelse(!is.na(ar08yr), paste0(ar08yr, "-", ar08mth),
                  ifelse(!is.na(birth_year_m), paste0(birth_year_m, "-", birth_month_m), NA)))
...
ggplot(parentscheck_biological, aes(x=siblingcount2, y=birthorder)) + geom_jitter()
...
parentscheck$death_year_m = ifelse(parentscheck$alive_m==3, ifelse(!is.na(parentscheck$death_year_m), parentscheck$death_year_m, parentscheck$birth_year_m+parentscheck$death_age_m), parentscheck$death_year_m)

individuals_parents_checked = subset(parentscheck, parentscheck$age>14)
